...
singleWordProb = OneGramInitializer('one-grams.txt')
...
@memoize
def wordSegmentor(word):
   if not word: return []
   word = word.lower()
   allSegmentations = [[first] + wordSegmentor(rest) for (first,rest) in splitPairs(word)]
